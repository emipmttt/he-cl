  ...
  <tr v-if="!deleted">
    ...
    <td>
      <button
        ...
        class="btn-flat indigo-text modal-trigger"
        @click="toEdit"
        title="Editar"
      >
      ...
      <div :id="'modalEdit_'+id" class="modal modal-fixed-footer">
          ...
          <form @submit.prevent="send">
            ...
            <div class="row">
              <div class="col s12 m6">
                <label>Rango</label>
                <select class="browser-default" v-model="rangeNum" required>
...
export default {
  ...
  methods: {
    send() {
      ...
      axios
        .post(
          "https://clima-laboral.human-express.com/php/textualRanges/update.php",
        ...
        .then(response => {
          ...
          if (response.status) {
            ...
            var instance = M.Modal.getInstance(
              document.getElementById("'modalEdit_'+id")
            );
            instance.close();
          } else {
    ...
    deleteElement() {
      if (confirm("Haz click en Aceptar para eliminar el rango")) {
        ...
        axios
          .post(
            "https://clima-laboral.human-express.com/php/textualRanges/delete.php",
            this.createFormData(data)
          )
          .then(element => {
            M.toast({
