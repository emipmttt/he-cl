    ...
    <div class="container">
      ...
      <form v-show="create" @submit.prevent="send" id="createReactive">
        ...
        <div class="input-field">
          ...
          <textarea id="answersList" maxlength="1000" class="materialize-textarea validate" v-model="answersList"
            required></textarea>
      ...
      <div v-if="reactives.length">
        <div>
          <reactive-item @update:change="get" v-for="(reactive, index) in filteredReactives" :key="reactive.id"
            :reactive=" reactive" />
        </div>
      </div>
