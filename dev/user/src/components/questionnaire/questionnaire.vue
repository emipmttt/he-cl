            ...
            <div v-if="!userCodeStatus">
              <form @submit.prevent="verifyUserCode" class="card-panel">
                <p class="left-align">
                  ...
                  <span
                    class="normal-text"
                  >Toda la información que proporciones es estrictamente Confidencial y Anónima</span>
                  <br>
                ...
                <div class="input-field col s6">
                  ...
                  <input id="userCode" type="password" class="validate" v-model="userCode">
              ...
              <div v-else>
                <div :key="index" v-for="(reactive,index) in reactives">
                  <div v-if="reactive.timer > 0">
                    <div
                      v-if="reactive.status == 1"
                      :id="'cover' + reactive.id"
                      @click="startQuestion(answersStringToObject(reactive.answersList)[0].aspect,reactive.timer,index)"
                      class="valign-wrapper card-panel indigo lighten-1"
                      style="min-height:80vh"
                    >
                      <div class="white-text center" style="margin:0 auto">
                        <i class="material-icons" style="font-size:6rem">timer</i>
                        <br>
                        <h2 class="large-text">Cronómetro</h2>
                        <p>Haz click aquí para mostrar la pregunta</p>
                        <p>
                          Tendrás
                          <strong>{{reactive.timer}}</strong> segundos para contestar
                        </p>
                      </div>
                    </div>
                    <div
                      v-else-if="reactive.status == 2"
                      :id="'question' + reactive.id"
                      class="card-panel"
                      style="min-height:80vh"
                    >
                      ...
                      <div style="margin:5% 0">
                        ...
                        La pregunta finalizará en
                        <strong
                          :id="'timer'+reactive.id"
                          class="indigo white-text"
                          style="border-radius: 1rem;padding:2px"
                        >{{reactive.timer}}</strong>
                        segundos
                      </div>
                      <form
                        @submit.prevent="answered(reactive.id,answer.aspect,answer.value)"
                        :key="index"
                        v-for="(answer, index) in answersStringToObject(reactive.answersList)"
                      >
                        <button
                          class="btn indigo waves-effect waves-light"
                          style="text-align:left !important;width:100%;margin-bottom:5px"
                        >{{answer.value}} {{answer.text}}</button>
                      </form>
                    </div>
                  </div>
                  <div
      ...
      axios
        ...
        .then(response => {
          ...
          campains.forEach(campain => {
            if (
              campain.title.toLowerCase() == this.displayTitle.toLowerCase()
            ) {
    ...
    addAspectsTotalList() {
      if (this.isCounted) {
        this.reactives.forEach(reactive => {
          let aspect = this.answersStringToObject(reactive.answersList)[0]
            .aspect;
